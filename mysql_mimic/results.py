...
class ResultSet:
    ...
    Args:
        rows: Rows to return to the client.
        columns: Column metadata. Empty for statements that don't return rows
            (e.g. INSERT/UPDATE/DELETE), in which case an OK packet is sent.
        affected_rows: Number of rows changed by a non-SELECT statement.
            Sent in the OK packet, where clients read it as `cursor.rowcount`.
            Ignored if the result has columns.
        last_insert_id: Value of AUTO_INCREMENT generated by an INSERT.
            Sent in the OK packet, where clients read it as `cursor.lastrowid`.
            Ignored if the result has columns.
    """
